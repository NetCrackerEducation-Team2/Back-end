...
create index users_achievements_achievement_id_index
    on users_achievements (achievement_id);

create index users_achievements_user_id_index
    on users_achievements (user_id);
...
(
    recommendation_id serial                  not null
        constraint recommendations_pk
            primary key,
    user_id           integer                 not null
        constraint recommendations_users
            references users,
    book_id           integer                 not null
        constraint recommendations_books
            references books,
    creation_time     timestamp default now() not null
);
